...
const OrdersTable = () => {
  ...
  useEffect(() => {
    dispatch(getOrders());
  }, [jwt, adminOrder.confirmed, adminOrder.shipped, adminOrder.delivered , adminOrder.deleteOrder]);
  ...
  return (
    <Box>
      ...
      {item.orderItems.map((orderItem)=><Avatar  alt={item.name} src={orderItem.product?.image} /> )}
    </AvatarGroup>
                    {" "}
                  </TableCell>
